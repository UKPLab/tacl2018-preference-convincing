...
class PreferenceComponents(object):
    ...
    def _init_invKf(self, p):
        if self.pref_gp[p].cov_type == 'diagonal':
            self.invKf[p] = np.eye(self.pref_gp[p].obs_coords.shape[0])
            return 
        
        if self.use_noise_svi or self.pref_gp[p].K is None or len(self.pref_gp[p].K) == 0:
        ...
        if self.new_obs:
            for person in self.people:
                self.pref_gp[person] = GPPrefLearning(self.nitem_features, self.mu0, self.shape_sf0, self.rate_sf0,
                                        ...
                                        use_svi=self.use_noise_svi, delay=self.delay, 
    ...
    def lowerbound(self):
        ...
        # this is not quite right as we are missing:-
        # - term involving cov(w), since logpw includes an E[ w^T w ] term.
        # - term involving lns_w, since logpw includes an E [ln|K^-1 * s_w|] term, which amounts to Elns_w rather than Es_w
        # - this also applies to y, t  
