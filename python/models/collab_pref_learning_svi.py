...
class CollabPrefLearningSVI(CollabPrefLearningVB):
    ...
    def _choose_inducing_points(self):
        ...
        if self.person_features is None:
            ...
            if self.y_ninducing >= self.Npeople:
                ...
                #                                             self.lsy,
                #                                             self.person_features[p:p + 1, :]
                #                                             )))

                # print distances to the coords
                # for p in range(self.person_features.shape[0]):
                #     print('%i: %f' % (p, total_corr(p)))
    ...
    def _estimate_obs_noise(self):
        ...
        # TODO: test latest tweaks on the experiments. We made the randomness in the samples depend only on the inducing
        # points as there is no other randomness in our approximate model, and we fixed a bug with nu0_total.
        # Previously we were using the Q2 setup, which ignores dependencies between points. This worked well on consensus
        # and Sushi A but was less good on Sushi B and personal. For those two, the higher Q3 was better.
        # Now we have corrected a few things, Q should be in between these values and hopefully work better for all expts.
        # However, the shape_s0 and rate_s0 values may need readjusting.
    ...
    def _update_sample_idxs(self, data_obs_idx_i=None, compute_y_var=True):
        ...
        self.y_cov_i = np.zeros((self.Nfactors, self.uy_i.shape[0], self.uy_i.shape[0]))
        ...
        if self.person_features is not None:
            ...
            sy = self.shape_sy / self.rate_sy # np.ones(self.Nfactors)#

            for f in range(self.Nfactors):
                self.y_cov_i[f] = Ky_i / sy[f] + covpair.dot(self.yS[f] - self.Ky_mm / sy[f]).dot(covpair.T)
        else:
            for f in range(self.Nfactors):
                self.y_cov_i[f] = np.diag(self.yS[f][self.uy_i])
    ...
    def lowerbound(self):
        ...
        logpy = np.sum([expec_pdf_gaussian(self.Ky_mm, self.invKy_mm, 0, self.y_ninducing, sy[f],
