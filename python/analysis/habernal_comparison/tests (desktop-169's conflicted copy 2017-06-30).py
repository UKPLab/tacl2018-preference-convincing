...
def run_test(folds, folds_regression, dataset, method, feature_type, embeddings_type=None, embeddings=None, 
             ...
             expt_tag='habernal', dataset_increment=0, acc=1.0, initial_pair_subset=None, max_no_folds=10, npairs=0,
             test_on_train=False):
    ...
    for foldidx, fold in enumerate(folds.keys()):
        ...
        if dataset_increment != 0:
            nnew_pairs = dataset_increment#int(np.floor(dataset_increment * npairs))
            # select the initial subsample of training pairs
            pair_subset = np.random.choice(npairs, nnew_pairs, replace=False)
        else:
            nnew_pairs = npairs
        ...
        while nseen_so_far < npairs:
            ...
            if tr_proba is not None:
                ...
                logging.info("Accuracy for fold = %f" % (np.sum(prefs_unseen[prefs_unseen != 1] == 2 * 
                    np.round(tr_proba).flatten()[prefs_unseen != 1]) / float(np.sum(prefs_unseen != 1))) )   
...
'''
def run_test_set(run_test_fun=run_test, subsample_tr=0, max_no_folds=10, npairs=0, test_on_train=False):
...
if __name__ == '__main__':
    acc = 1.0
    dataset_increment = 0#100
    
#     datasets = ['UKPConvArgCrowdSample_evalMACE']
#     methods = ['SinglePrefGP_noOpt_weaksprior_additive']
#     feature_types = ['both']
#     embeddings_types = ['word_mean']
#     default_ls_values = run_test_set(max_no_folds=10, npairs=1000)
