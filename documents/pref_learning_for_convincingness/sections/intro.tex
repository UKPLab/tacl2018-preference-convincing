...
Empirical approaches instead require example judgements of arguments,
such as those shown in Figure \ref{fig:argument_examples}, to train machine learning methods.
%Consider the :
%how does one assign a numerical convincingness score to each argument? 
%arguments paired with human judgements of their convincingness.
...
a very restricted set of categories, or %that restricts filtering or sorting of arguments, or
the need for multiple annotators per text.
...
For example, machine learning typically requires datasets with 
