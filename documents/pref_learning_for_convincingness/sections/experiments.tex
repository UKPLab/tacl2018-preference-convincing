...
%We extended the BiLSTM approach to accommodate argument-level features as well as word sequences by adding a %fully-connected layer with $64$ units.
...
\subfloat[\;no cycle]{
  \includegraphics[width=.22\columnwidth, clip=True, trim=60 60 45 24]{figures/cycles_demo/no_cycle/GPPL_probas}
}
\subfloat[single cycle]{
  \includegraphics[width=.205\columnwidth, clip=True, trim=70 60 48 24]{figures/cycles_demo/simple_cycle/GPPL_probas}
}
\subfloat[double cycle]{
  \includegraphics[width=.205\columnwidth, clip=True, trim=70 60 48 24]{figures/cycles_demo/double_cycle/GPPL_probas}
}
\subfloat[9 undecided\;\;\;\;\;]{
  \includegraphics[width=.27\columnwidth, clip=True, trim=60 60 5 24]{figures/cycles_demo/undecided/GPPL_probas_2}
}\\
...
%\centering
\subfloat[]{
  \includegraphics[width=.22\columnwidth, clip=True, trim=60 45 45 24]{figures/cycles_demo/no_cycle/SVM_probas} 
}
\subfloat[]{
  \includegraphics[width=.205\columnwidth, clip=True, trim=70 45 48 24]{figures/cycles_demo/simple_cycle/SVM_probas} 
}
\subfloat[]{
  \includegraphics[width=.205\columnwidth, clip=True, trim=70 45 48 24]{figures/cycles_demo/double_cycle/SVM_probas} 
}
\subfloat[]{
  \includegraphics[width=.27\columnwidth, clip=True, trim=60 45 5 24]{figures/cycles_demo/undecided/SVM_probas} 
}
...
\includegraphics[width=0.55\columnwidth, clip=True, trim=12 54 12 10]{figures/scalability/num_arguments.pdf}
\label{fig:scale_N}
}
\hspace{0.1cm}
\subfloat[Varying no. ling+Glove features, GPPL, medi., M=500]{
\includegraphics[width=0.55\columnwidth, clip=True, trim=0 47 25 10]{figures/scalability/num_features_gppl.pdf}
\label{fig:scale_dims}
}
\hspace{0.1cm}
\subfloat[Varying no. ling+Glove features, long-running methods]{
\includegraphics[width=0.59\columnwidth, clip=True, trim=0 58 14 10]{figures/scalability/num_features_others.pdf}
...
\hspace{-1.5mm}
\includegraphics[width=0.49\columnwidth, clip=True, trim=4 40 15 0]{figures/scalability/num_inducing_32310_features.pdf}
\label{fig:scale_M_b}}
\subfloat[300 Glove features]{
\hspace{-1.5mm}
\includegraphics[width=0.48\columnwidth, clip=True, trim=10 40 17 0]{figures/scalability/num_inducing_300_features.pdf}
\label{fig:scale_M_a}}
...
%PageRank and GPPL produce very different results. Here, the argument graph shows 
...
%, we were unable to test the optimisation
...
%The informativeness of production rules would be reduced if high quality and low quality arguments share common grammatical structures.
...
%However, the small changes to the 
...
%suggesting that some of these features are far more informative for argument convincingness than others.
...
%only very weakly informative in comparison to other dimensions of the embedding %vectors. 
...
%In future, features with large length-scales could be discarded to reduce the computational cost of training and prediction. 
