...
\toprule
Dataset & \#folds/ & \#users & \#items & train & \multicolumn{2}{c}{test} & \multicolumn{2}{c}{\#features} \\
...
We test the key aspects of our proposed methods using
 the datasets summarized in Table \ref{tab:datasets}: 
 recovering an underlying consensus from noisy pairwise labels in a subjective task; modeling personal preferences from pairwise labels; and the scalability of our proposed Bayesian preference learning methods, GPPL and crowdGPPL using SVI.
...
\begin{table}
 ...
 \small
 \setlength{\tabcolsep}{4pt}
\begin{tabular}{ l l l l@{\hskip 0.8cm} l l l@{\hskip 0.8cm} l l l }
\hline
 & \multicolumn{3}{l}{Consensus}&\multicolumn{3}{l}{Personal: all workers} &\multicolumn{3}{l}{$>$40 training pairs} \\
 Method & Acc & CEE & $\tau$ & Acc & CEE & $\tau$ & Acc & CEE & $\tau$ \\ 
  \midrule
 ...
 \\\hline
\end{tabular}
\caption{UKPConvArgCrowdSample,  using ling+GloVe features: predicting consensus values, personal preferences,
and personal preferences for workers with >40 pairs in the training set.
