...
$s^{(v)}_c = 1,s^{(w)}_c = 1$, for all components.
...
The second simulation modifies the previous setup by fixing $s^{(t)} = 1,s^{(w)}_c=1,\forall c$ and varying $s^{(v)}_c,\forall c$
...
\end{figure}
\todo{
ADD the updated plots when tests are complete.
...
We hypothesise that learning a model with a larger number of latent components
requires more training data.
%scenario that would require more training data. 
In the third simulation, we generate data using the same setup as before, but fix $s^{(v)}_c=s^{(w)}=1,\forall c$ and $s^{(t)}=1$
...
However, for all values of $C_{true}$, the correlation with $>500$ labels remains above 0.5,
...
However, as the EP approach of collabGP has been consistently shown to perform well for non-Gaussian likelihoods\citep{nickisch2008approximations},
we consider its performance as a target for our SVI approach for crowdGPPL.
\todo{merge this up with other stuff on collabGP}
...
For $\backslash\bs u$ methods, the user covariance matrix, $\bs L$, 
in the crowdGPPL model is replaced by the identity matrix, 
and for crowdGPPL$\mathbf{\backslash \bs u \backslash \bs x}$, 
the item covariance matrices, $\bs K_v$ and $\bs K_t$ are also replaced by the identity matrix.
...
$\epsilon=5$, $P_i=200$ for \emph{Sushi-A} and $P_i=2000$ for \emph{Sushi-B},
without optimisation.
The gamma hyperparameters for all models are $\alpha_0=1, \beta_0=100$, 
using a grid search over values 
$10^{\{-1,...,3\}}$ on withheld user data from the \emph{Sushi-A} dataset,
except for $\alpha_0^{(w)}=1,\beta_0^{(w)}=1$.
...
illustrate the performance benefit of crowd models over single-user GPPL.
%and
%the runtimes 
...
% runtimes of these methods over joint-GPPL. 
 ...
 compared to CrowdGPPL$\backslash\bs u$ and crowdGPPL$\backslash\bs u\bs x$,
 except on Sushi-A-small, where they may help with the small amount of training data.
  both outperform the full crowdGPPL. 
...
Note that the consensus helps predicting workers with few labels.}
\todo{Make sure the accuracies for crowdGPPL are correct with optimal setup. Make sure the tau for > 40 is correct.
Check that GPPL accuracies and tau for personal are correct. 
What we need: doesn't matter what GPPL is like, as long as the consensus prediction is close to previous work. 
crowdGPPL should be better than all the others.
The length-scale multiplier for all methods should be the same, i.e. 20 x D at the moment,
so ideally we rerun all. The best setup could
be tested with 10 and 15 for crowdGPPL.
}
