...
\subfloat[Consensus]{
\label{fig:simB}
\includegraphics[width=.322\columnwidth,clip=true,trim=11 0 13 0]{../../results/synth_sandbox/multi_user_consensus/tau_test}
}
\subfloat[Personal preferences]{
\label{fig:simC}
\includegraphics[width=.316\columnwidth,clip=true,trim=20 0 13 0]{../../results/synth_sandbox/multi_user_personal/tau_test}
}
\subfloat[Latent factors]{
\label{fig:simD}
\includegraphics[width=.325\columnwidth,clip=true,trim=11 0 12 0]{../../results/synth_3/multi_factor_correlations_P/num_pairs_r}
}
\caption{Simulations: Rank correlation between true and inferred preference values for different inference tasks.  (a) \& (b) varying level of noise in pairwise training labels, (d) varying number of pairwise training labels. 
}
\end{figure}
...
use Mat\'ern 3/2 kernels with length-scales chosen by a median heuristic:
\begin{flalign}
...
which normalizes features and maintains the average covariance between items or users
...
We hypothesize that generating models with a larger number of latent components
require more training data to learn
...
\hline
 ...
 Method & Acc & CEE & $\tau$ & Acc & CEE & $\tau$ \\ \hline
 %SVM & .70 & .58 & .31 & .63 & .66 & .31 \\
 %Bi-LSTM & .73 &  .55 & .21 & .64 & .64 & .21 \\
 GPPL %medi.
  & .77 & .50 & .40 & .72 & .55 & .26 \\ % .71 & .56 & .32 \\ % now showing with > 40 training examples
 %GPPL opt. & .76 & .51 & .47 & .70 & .58 &  .30 \\
 crowdGPPL %medi. 
 & .78 & .48 & .51 & .73 & .56 & .28  %.71 & .59 & .32 \\ % now showing workers with > 40 training examples
...
\emph{UKPConvArgCrowdSample} was subsampled by~\citet{simpson2018finding}
...
We predict both the consensus and personal preferences: we avoid making predictions for new workers by evaluating the personal predictions
only on workers who provided $>40$ pairwise labels in the training data for each fold.
...
except for: $\alpha_0 = 2$, $\beta_0 = 200$, set by comparing
training set accuracy of GPPL 
against $\alpha_0=2, \beta_0 = 20000$ and $\alpha_0 = 2, \beta_0 = 2$;
and $C=50$ and $\epsilon=2$, which were not optimized.
...
With $33,210$ features, the runtime includes large overheads due to the computation of
...
Increasing the number of pairwise labels, $P$, however does not affect the runtimes to a large degree.
...
\label{fig:M}
 \includegraphics[clip=true,trim=10 0 0 0,width=.38\columnwidth]{../../results/scalability/num_inducing_32310_features}
}
\subfloat[Varying number of items, $N$.]{
\label{fig:Ntr}
\includegraphics[clip=true,trim=10 0 10 0,width=.32\columnwidth]{../../results/scalability/num_arguments}
}
\subfloat[Varying no. pairwise training labels, $P$. ]{
\label{fig:Npairs}
\includegraphics[clip=true,trim=50 0 10 0,width=.27\columnwidth]{../../results/scalability/num_pairs}
}
\caption{
...
To test performance with very few training pairs, we obtain Sushi-A-small
by selecting $100$ users at random from the complete $Sushi-A$ dataset,
then selecting $5$ pairs for training and $25$ for testing per user.
For Sushi-A, we select a subset of $1000$ users at random from the complete dataset, then 
...
$10^{\{-1,...,3\}}$ on withheld user data from the Sushi-A dataset,
$\epsilon=5$, $|P_i|=200$ for Sushi-A and $|P_i|=2000$ for Sushi-B. All others are the same 
...
The runtimes 
show the benefit of the sparse approximation over crowdGPPL$\backslash$induc.
...
% runtimes of these methods over joint-GPPL. 
 ...
 When using inducing points, the user features decrease the performance of crowdGPPL on Sushi-A and Sushi-A-small: crowdGPPL$\backslash$induc
 ...
 However, on Sushi-B, the user features provide a small improvement, so may be weakly informative
 given enough users and in the case where we have only $10$ pairwise labels per users, rather than $15$ as for Sushi-A.
 ...
 GPPL-per-user performs well on Sushi-A, but does not perform as well as other methods on Sushi-B, as
...
CrowdGPPL produces similar classification scores to the earlier method of 
 \citet{houlsby2012collaborative} (0.83 on Sushi-A with user features, and 0.84 without), 
 while using a more scalable inference method.
Performance is also improved over \citet{khan2014scalable} , 
whose model comprised a GP for each user and matrix factorization (CEE=0.69 on Sushi-B),
 as well as \citet{salimans2012collaborative}, using matrix factorization with no user or item features (Kendall's $\tau\approx0.39$ on Sushi-B). 
...
to the many of the model's predictions. This indicates that our Bayesian approach, in which the priors
...
\includegraphics[trim=10 10 0 0,clip=true,width=.34\textwidth]{../../results/conv_factors/conv_factor_scales.pdf}
}
\subfloat[Sushi-A]{
\includegraphics[trim=20 5 0 0,clip=true,width=.32\textwidth]{../../results/sushi_factors/sushi_factor_scales}
}
\caption{
