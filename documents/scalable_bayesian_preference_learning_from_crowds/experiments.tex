...
determine kernel length-scales using the median heuristic,
 and for crowdGPPL set $C=25$. The SVI hyperparameters were set to 
...
%}
\caption{
Effect of varying $M$ on accuracy and runtime (training+prediction) for GPPL and crowdGPPL on UKPConvArgCrowdSample. Means over 15 runs.
}
\label{fig:M}
\end{figure}
\begin{figure}
...
\label{fig:Ntr}
\includegraphics[width=.478\columnwidth]{../../results/scalability/num_arguments}
}
\subfloat[Varying no. pairwise training labels. ]{
\label{fig:Npairs}
\includegraphics[clip=true,trim=20 0 0 0,width=.44\columnwidth]{../../results/scalability/num_pairs}
}
\caption{
    Runtimes for training+prediction on UKPConvArgCrowdSample with varying subsample size. 300 GloVe features. Means over 15 runs. CrowdGPPL with 5 components.  Note the logarithmic x-axis for (b).
}
\end{figure}
...
\includegraphics[trim=10 10 0 0,clip=true,width=.42\textwidth]{../../results/conv_factors/conv_factor_scales.pdf}
}
\subfloat[Sushi-A]{
\includegraphics[trim=20 5 0 0,clip=true,width=.4\textwidth]{../../results/sushi_factors/sushi_factor_scales}
}
\caption{
