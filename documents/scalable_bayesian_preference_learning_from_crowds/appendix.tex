...
\begin{flalign}
\mathcal{L}_1 = \sum_{i=1}^{L} \mathbb{E}_{q}\left[ \ln p\left( v_i \succ u_i | f(v_i), f(u_i) \right) \right]
+ \mathbb{E}_{q}\left[ \ln \frac{p\left( \bs f | \bs\mu, \bs K/s \right)}{q\left(\bs f\right)} \right]
+ \mathbb{E}_{q}\left[ \ln \frac{p\left( s | \alpha_0, \beta_0\right)}{q\left(s \right)} \right] &&
\label{eq:vblb}
\end{flalign}
Substituting the forms of the distributions with their variational parameters, we get:
\begin{flalign}
\mathcal{L}_1 & = \mathbb{E}_{q}\Bigg[ \sum_{i=1}^{L} [v_i \succ u_i]\ln\Phi(z_i) + [v_i \prec u_i]\left(1-\ln\Phi(z_i)\right) \Bigg] \nonumber&&\\
& + \ln \mathcal{N}\left(\hat{\bs f}; \bs\mu, \bs K/\hat{s} \right) 
- \ln\mathcal{N}\left(\hat{\bs f}; \hat{\bs f}, \bs C \right)
 + \mathbb{E}_{q}\left[ \ln\mathcal{G}\left( s; \alpha_0, \beta_0\right) - \ln\mathcal{G}\left(s; \alpha, \beta \right) \right]  &&
\end{flalign}
...
\mathcal{L}_1 \approx \mathcal{L}_2 & = \mathbb{E}_{q}\left[ \mathcal{N}( \bs y | \Phi(\bs z), \bs Q) \right]
 + \ln \mathcal{N}\left(\bs f; \bs\mu, \bs K/\hat{s} \right) - \ln\mathcal{N}\left(\bs f; \hat{\bs f}, \bs C \right) 
&\nonumber\\
& + \mathbb{E}_q\left[ \ln\mathcal{G}\left( s; \alpha_0, \beta_0\right) - \ln\mathcal{G}\left(s; \alpha, \beta \right) \right] \nonumber&\\
& =  - \frac{1}{2} \left\{ L \ln 2\pi + \ln |\bs Q| - \ln|\bs C| 
 + \ln|\bs K/s| + (\hat{\bs f} - \bs\mu)\hat{s}\bs K^{-1}
...
& + \Gamma(\alpha) + (\beta-\beta_0) \hat{s} - \alpha \ln \beta,  &
\end{flalign}
...
\mathcal{L}_2 & \approx \mathcal{L}_3 = - \frac{1}{2} \left\{ L \ln 2\pi + \ln |\bs Q| - \ln|\bs C| \right.
 \left. + \ln|\bs K/\hat{s}| + (\hat{\bs f} - \bs\mu)\hat{s}\bs K^{-1}(\hat{\bs f} - \bs\mu) \right. \nonumber&&\\
...
\begin{flalign}
\mathcal{L} \approx \mathcal{L}_3 =\; & \mathbb{E}_{q(\bs f, \bs f_m, s)}[\ln p(\bs y | \bs f) + \ln p(\bs f_m, s | \bs K, 
\alpha_0, \beta_0) -\ln q(\bs f_m) - \ln q(s) ] & \nonumber \\ 
=\; & \sum_{p=1}^P \mathbb{E}_{q(\bs f)}[\ln p(y_p | f_{a_p}, f_{b_p})] - \frac{1}{2} \bigg\{ \ln|\bs K_{mm}| - \mathbb{E}[\ln s] - \ln|\bs S| - M
\nonumber &\\
...
\sum_{p=1}^P \mathbb{E}_{q(\bs f)}[\ln p(y_p | \bs v_{a_p}^T \! \bs w_{a_p} \!+ t_{a_p}\!, \bs v_{b_p}^T\! \bs w_{b_p} \!+ t_{b_p})] 
- \frac{1}{2} 
\Bigg\{  \sum_{c=1}^C \bigg\{  \! - \! M_n \! - \! M_u & \nonumber \\
&  
 + \ln|\bs K_{v,mm}| + \ln|\bs K_{w,mm}|
- \ln|\bs S_{v,c}|  - \mathbb{E}[\ln s_c] 
+ \hat{\bs v}_{m,c}^T \mathbb{E}[s_c]\bs K_{v,mm}^{-1}\hat{\bs v}_{m,c} & \nonumber \\
& 
+ \textrm{tr}(\mathbb{E}[s_c] \bs K_{v,mm}^{-1} \bs S_{v,c}) 
- \ln|\bs \Sigma_{c}|  + \hat{\bs w}_{m,c}^T \bs K_{w,mm}^{-1}\hat{\bs w}_{m,c} 
+ \textrm{tr}(\bs K_{w,mm}^{-1} \bs \Sigma_{c})
\bigg\}
& \nonumber \\
&  
- M_n + \ln|\bs K_{t,mm}|
- \ln|\bs S_{t}|  - \mathbb{E}[\ln \sigma] 
...
& \nonumber \\
& + \sum_{c=1}^C \bigg\{ 
\ln\Gamma(\alpha_c) + (\alpha_0 - \alpha_c)\mathbb{E}[\ln s_c]
+ (\beta_c - \beta_0) \mathbb{E}[s_c] - \alpha_c \ln \beta_c \bigg\}
 & 
\nonumber \\ 
& + \ln\Gamma(\alpha_{\sigma}) + (\alpha_0 - \alpha_{\sigma})\mathbb{E}[\ln \sigma]
+ (\beta_{\sigma} - \beta_0) \mathbb{E}[s_c] - \alpha_{\sigma} \ln \beta_{\sigma}
, &
\end{flalign}
...
% % \frac{\partial \ln|\bs K/\hat{s}|}{\partial l_d} - \frac{\partial \ln|\bs C|}{\partial l_d} 
% % \nonumber \right.
% %  \left.  - (\hat{\bs f}-\bs\mu)\hat{s} \frac{\partial K^{-1}}{\partial l_d} (\hat{\bs f}-\bs\mu)
% % \right\rbrace \nonumber & \\
% % %& = \frac{1}{2} \hat{s} \left\lbrace \frac{\partial \ln |\bs C \bs K^{-1}|}{\partial l_d}
% % %\right. \\
% % %& \left.  - (\hat{\bs f}-\bs\mu) \bs K^{-1} \frac{\partial \bs K}{\partial l_d} \bs K^{-1} (\hat{\bs f}-\bs\mu)
% % %\right\rbrace  \nonumber \\
% % & =  -\frac{1}{2} \left\lbrace  \frac{\partial \ln | \frac{1}{\hat{s}}\bs K \bs C^{-1} |}{\partial l_d} \right. 
