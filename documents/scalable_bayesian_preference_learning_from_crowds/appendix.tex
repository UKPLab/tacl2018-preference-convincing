...
\begin{flalign}
\mathcal{L}_1 = \sum_{p=1}^{P} \mathbb{E}_{q(\bs f)}\left[ \ln p\left( y(a_p, b_p) | f(\bs x_{a_p}), f(\bs x_{b_p}) \right) \right]
+ \mathbb{E}_{q(\bs f),q(s)}\left[ \ln \frac{p\left( \bs f | \bs 0, \bs K/s \right)}
...
\begin{flalign}
\mathcal{L}_1 & = \mathbb{E}_{q(\bs f)}\Bigg[ \sum_{p=1}^{P} y(a_p,b_p) \ln\Phi(z_p) + y(b_p,a_p) \left(1-\ln\Phi(z_p)\right) \Bigg] \nonumber&&\\
& + \ln \mathcal{N}\left(\hat{\bs f}; \bs\mu, \bs K/\mathbb{E}[s] \right) 
...
\begin{flalign}
\mathcal{L} \approx \mathcal{L}_3 =\; & \mathbb{E}_{q(\bs f)}[\ln p(\bs y | \bs f)]
...
+ \hat{\bs v}_{m,c}^T \mathbb{E}[s_c]\bs K_{mm}^{-1}\hat{\bs v}_{m,c} 
+ \textrm{tr}(\mathbb{E}[s_c] \bs K_{mm}^{-1} \bs S_{v,c}) 
& \nonumber \\
& 
- \ln|\bs \Sigma_{c}|  + \hat{\bs w}_{m,c}^T \bs L_{mm}^{-1}\hat{\bs w}_{m,c} 
+ \textrm{tr}(\bs L_{mm}^{-1} \bs \Sigma_{c})
\bigg\}
- M_n + \ln|\bs K_{mm}|
- \ln|\bs S_{t}|  - \mathbb{E}[\ln s_t] 
& \nonumber \\
&  
+ \hat{\bs t}^T \mathbb{E}[s_t] \bs K_{mm}^{-1} \hat{\bs t} 
+ \textrm{tr}(\mathbb{E}[s_t] \bs K_{mm}^{-1} \bs S_{t})
\Bigg\} 
...
+ (\beta_c - \beta_0) \mathbb{E}[s_c] - \alpha_c \ln \beta_c \bigg\}
 & 
\nonumber \\ 
& + \ln\Gamma(\alpha_{s_t}) + (\alpha_0 - \alpha_{s_t})\mathbb{E}[\ln s_t]
+ (\beta_{s_t} - \beta_0) \mathbb{E}[s_c] - \alpha_{s_t} \ln \beta_{s_t}
, &
\end{flalign}
...
\begin{flalign}
\bs S_{v,c,i}^{-1} & = (1-\rho_i)\bs S^{-1}_{v,c,i-1} + \rho_i\left( \bs K^{-1}_{mm}\mathbb{E}[s_c] 
 + \pi_i \bs A_{v,i}^T \bs G_i^T \textrm{diag}(\hat{\bs w}_{c,\bs u}^2 + \bs\Sigma_{c,\bs u,\bs u})\bs Q_i^{-1} \bs G_i \bs A_{v,i} \right), &
\label{eq:Sv}
\end{flalign}
...
(1-\rho_i)\bs S^{-1}_{v,c,i-1}\hat{\bs v}_{m,c,i-1} + \rho_i \pi_i \bigg(
...
\begin{flalign}
\bs S_{t,i}^{-1} = \;\;& (1-\rho_i)\bs S^{-1}_{t,i-1} + \rho_i\bs K^{-1}_{t,mm}/\mathbb{E}[s_t] 
+ \rho_i \pi_i \bs A_{i}^T \bs G_i^T \bs Q_i^{-1} \bs G_i \bs A_{i} & \label{eq:St}\\
\hat{\bs t}_{m,i} = \;\;& \bs S_{t,i}\left(
(1 - \rho_i) \bs S_{t,i-1}^{-1}\hat{\bs t}_{m,i-1}  
...
\hat{\bs w}_{c}^* = \bs L_{*m} \bs L^{-1}_{mm} \hat{\bs w}_{m,c}, & \label{eq:tstar}\\
&\bs C_{t}^* \!= \frac{\bs K_{**}}{\mathbb{E}[s_t]} + \bs A_{*m}(\bs S_{t} \!-\! \bs K_{mm}) \bs A_{*m}^T, \hspace{0.5cm}
