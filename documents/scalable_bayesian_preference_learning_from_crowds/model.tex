...
are the utilities of the $N$ items referred to by $\bs y$.%, and $\theta$, $\alpha_0$ and $\beta_0$ are hyper-parameters.
...
\textrm{where } z_p = \bs v_{.,a_p}^T \bs{w}_{.,u_p} + t_{a_p} - \bs v_{.,b_p}^T \bs{w}_{.,u_p} - t_{b_p} , && 
