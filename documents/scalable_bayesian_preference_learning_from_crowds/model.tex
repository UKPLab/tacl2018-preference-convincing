...
For a pair of items, $a$ and $b$, we write $a \succ b$ to symbolize that $a$ is preferred to $b$.  
A pairwise label has value $y(a, b) =1 $ if $a \succ b$ and $0$ otherwise.
% %\begin{equation}
% $y(a, b) = \begin{cases}
% 1 & a \succ b \\
% 0 & b \succ a
...
The uncertainty in the likelihood $p(y(a,b)=1)$ accommodates
 labeling errors %or noise in implicit annotations, such as click streams, 
 as well as variability in a user's judgments.
Here, we assume the utility to be a function of the items' features,
$f(\bs x_a) : \mathbb{R}^D \mapsto \mathbb{R}$, where $\bs x_a$ is a vector representation of the features of item $a$
of length $D$.
Pairwise labels can be modeled using a logistic likelihood %relationship between the utility function, $f$, and the pairwise labels
...
one for users, $\bs{W} \in \mathbb{R}^{C \times U}$, 
and one for the items, $\bs{V} \in \mathbb{R}^{C \times N}$,
...
The column $\bs v_{.,a}$ of $\bs V$, and the column $\bs w_{.,j}$ of $\bs W$,
 are latent vector representations of item $a$ and a user $j$,
 respectively.
...
For the multi-user case, we assume that there are $C$ latent functions of item features, $v_c\sim \mathcal{GP}(\bs 0, k_{\theta} /s_c)$, 
 and $C$ latent functions of user features, $w_c \sim \mathcal{GP}(\bs 0, k_{\eta})$,
 where $k$ is a kernel function, $s_c$ is an inverse scale for the item features,
 $\theta$ are the kernel hyperparameters for the items and $\eta$ are the kernel 
 hyperparameters for the users.
...
$t\sim \mathcal{GP}(\bs 0, k_{\theta} /s_t)$, that is shared across all users,
...
\begin{flalign}
p\left( \bs{y}, \bs V, \bs W, \bs t, s_1, ..., s_C, s_t | k_{\theta}, k_{\eta}, \alpha_0, \beta_0 \right) 
= \prod_{p=1}^P \Phi\left( z_p \right) 
\mathcal{N}(\bs t; \bs 0, \bs K_{\theta} /s_t)
 && \nonumber \\ 
\mathcal{G}({s_t}; \alpha_0, \beta_0)\prod_{c=1}^C \left\{
\mathcal{N}(\bs v_c; \bs 0, \bs K_{\theta} /s_c) 
\mathcal{N}(\bs w_c; \bs 0, \bs L_{\eta}) \mathcal{G}(s_c; \alpha_0, \beta_0) \right\}, 
\label{eq:joint_crowd}
\end{flalign}
where 
$z_p = \bs v_{.,a_p}^T \bs{w}_{.,u_p} + t_{a_p} - \bs v_{.,b_p}^T \bs{w}_{.,u_p} - t_{b_p}$,
 $s_t$ is the inverse scale of $t$,
