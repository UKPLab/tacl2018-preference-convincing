...
\begin{flalign}
& \ln q(s) = \ln\mathcal{N}(\mathbb{E}[\bs f_m| \bs 0, \bs K_{mm}/s] + \ln \mathcal{G}(s; \alpha_0, \beta_0) + \mathrm{const}
...
\begin{flalign}
\mathcal{L}_{cr} & = 
\mathbb{E}_{q(\bs F)}%(\bs t, \bs t_m, \bs V, \bs V_m, \bs W, \bs W_m, s_1,...,s_c,\sigma)
[\ln p(\bs y | \bs F)] 
+ \mathbb{E}_{q(\bs t_m), q(\sigma)}[\ln p(\bs t_m, \sigma | \bs K_{mm}, \alpha_0, \beta_0)
- \ln q(\bs t_m)] & \nonumber \\
&\sum_{c=1}^C \!\! \bigg\{ 
\mathbb{E}_{q(\bs v_{m,c}), q(s_c)}[\ln p(\bs v_{m,c}, s_c | \bs K_{mm}, \alpha_0, \beta_0) - \ln q(\bs v_{m,c})]
&  \nonumber \\ 
 & +  \mathbb{E}_{q(\bs w_{m,c})}[\ln p(\bs w_{m,c} | \bs L_{mm} )
  - \ln q(\bs w_{m,c} ) ] \bigg\} . & 
...
\begin{flalign}
\ln q(\bs v_{m,c}) = \;\;& \mathbb{E}_{q(\bs F)}\left[\ln \mathcal{N}\left( \bs y; \tilde{\Phi}(\bs z), Q \right)\right] 
 + \ln\mathcal{N}\left(\bs v_{m,c}; \bs 0, \frac{\bs K_{v,mm}}{\mathbb{E}[s_c]}\right) 
+ \textrm{const} & \nonumber \\
...
\todo{Add the subscripts to the expectations. Change from the sum over W to a single component}
\todo{Why not change the $K_mm$ subscripts to $K'$ superscript? It would save a lot of space and get rid of some double subsripts.}
\begin{flalign}
\ln q(\bs t_m) = \;\;& \mathbb{E}[\ln \mathcal{N}\left( \bs y; \tilde{\Phi}(\bs z), Q \right)] 
+ \mathbb{E}[\ln\mathcal{N}(\bs t_m; \bs 0, \bs K_{t,mm}/s)] 
+ \textrm{const} & \nonumber \\
= \;\;& \ln \mathcal{N}\left( \bs t; \hat{\bs t}, \bs S_t \right), &
\end{flalign}
...
\begin{flalign}
\ln q(\bs W_m) = \;\;& \mathbb{E}\left[\ln \mathcal{N}\left( \bs y; \tilde{\Phi}(\bs z), Q \right)\right] 
+ \sum_{c=1}^C \mathbb{E}[\ln\mathcal{N}(\bs w_c; \bs 0, \bs L_{mm})]
+ \textrm{const} & \nonumber \\
= \;\;& \sum_{c=1}^C \ln \mathcal{N}\left( \bs w_c; \hat{\bs w}_c, \bs \Sigma_c \right), & 
\end{flalign}
...
% \ln q(\bs W) & = \sum_{c=1}^C \ln \mathcal{N}\left( \bs w_c; \bs A_w \hat{\bs w}_{m,c}, \bs L_{} + \bs A_w (\bs\Sigma - \bs L_{mm}) \bs A_w \right). &
% \label{eq:qw}
% \end{flalign}
