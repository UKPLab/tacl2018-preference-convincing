...
& \hspace{3.7cm} q(\bs V)q(\bs W)q(\bs t)q(\bs V_m)q(\bs W_m)q(\bs t_m)\prod_{c=1}^{C}q(s_c)q(\sigma), &
\end{flalign}
...
\left[ 
\log p(\bs y | \bs F) +
\sum_{c=1}^C \big\{ \log p(\bs v_{m,c}, s_c | \bs K_{mm}, \alpha_0, \beta_0) - \log q(\bs v_{m,c}) \right. & \\
& \left. + \log p(\bs w_{m,c} | \bs L_{,mm} )
  - \log q(\bs w_{m,c} ) \big\}
...
+ \sum_{c=1}^C \mathbb{E}[\log\mathcal{N}(\bs w_c; \bs 0, \bs L_{,mm})]
+ \textrm{const} & \nonumber \\
= \;\;& \sum_{c=1}^C \log \mathcal{N}\left( \bs w_c; \hat{\bs w}_c, \bs \Sigma_c \right), & 
\end{flalign}
...
% \log q(\bs W) & = \sum_{c=1}^C \log \mathcal{N}\left( \bs w_c; \bs A_w \hat{\bs w}_{m,c}, \bs L_{} + \bs A_w (\bs\Sigma - \bs L_{,mm}) \bs A_w \right). &
% \label{eq:qw}
% \end{flalign}
