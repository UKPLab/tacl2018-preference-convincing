...
\begin{flalign}
\mathcal{L}_{cr} & = 
\mathbb{E}_{q(\bs F)}%(\bs t, \bs t_m, \bs V, \bs V_m, \bs W, \bs W_m, s_1,...,s_c,s_t)
[\ln p(\bs y | \bs F)] 
+ \mathbb{E}_{q(\bs t_m), q(s_t)}[\ln p(\bs t_m, s_t | \bs K_{mm}, \alpha_0, \beta_0)
- \ln q(\bs t_m)] & \nonumber \\
&\sum_{c=1}^C \!\! \bigg\{ 
...
\ln \mathcal{N}\left( \bs y; \tilde{\Phi}(\bs z), Q \right) % \right] 
 + \ln\mathcal{N}\left(\bs v_{m,c}; \bs 0, \bs K_{mm}/ \mathbb{E}[s_c]\right) 
+ \textrm{const} & \nonumber \\
...
+ \ln\mathcal{N}(\bs t_m; \bs 0, \bs K_{mm}/\mathbb{E}[s_t])
+ \textrm{const} & \nonumber \\
= \;\;& \ln \mathcal{N}\left( \bs t_m; \hat{\bs t}_{m}, \bs S_t \right), &
\end{flalign}
...
\ln \mathcal{N}\left( \bs y; \tilde{\Phi}(\bs z), Q \right) %\right] 
+ \ln\mathcal{N}(\bs w_{m,c}; \bs 0, \bs L_{mm})
+ \textrm{const} & \nonumber \\
...
% \frac{\bs K_{v}}{\mathbb{E}[s_c]} + \bs A_v (\bs S_{m,c} - \frac{\bs K_{mm}}{\mathbb{E}[s_c]})\bs A_v \right) & \label{eq:qv} \\
% \ln q(\bs t) & = \ln \mathcal{N}\left( \bs t; \bs A_t \hat{\bs t}_m, 
% \frac{\bs K_{t}}{\mathbb{E}[s_t]} + \bs A_t (\bs S_t - \frac{\bs K_{mm}}{\mathbb{E}[s_t]})\bs A_t \right)  & \label{eq:qt}\\
