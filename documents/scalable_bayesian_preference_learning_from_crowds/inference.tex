...
\todo{Do we actually state the complexity of our approach anywhere?}
...
\todo{ 
%got really confused by Section 4.1.  In equation (7), p(y|f) is a product of the cdfs.  
%that is correct.

%Where does the expectation come from just after the first equal sign? 
%fuck knows. I think that doesn't belong here, but should be stated later (it's already in eq 10, is that all we need?)

%The second equal sign in (7) should be an approximation.  
% I think what is missing is an approx after the first expectation, then the same thing with the expectation just inside the product. But why it here at all?

In equation (7), Q is (approximately) the covariance of y conditioned on f and yet, in equation (8), Q is determined by the expectation over f.  
% Need to explain: the variance of the beta is a function of its mean and the number of observations. So we to estimate this variance, we need to know the mean of f!
% But i think eq 9 is somehow not quite right.

Can you please check this section and provide some further explanation.}
...
is not conjugate with the Gaussian process, which makes the exact posterior intractable.
We therefore
...
\begin{flalign}
p(\bs y | \bs f) = \prod_{p=1}^P p(y_p | z_p)
= \prod_{p=1}^P \Phi(z_p)
%= \mathbb{E}\left[\prod_{p=1}^P \Phi(z_p)\right] = \prod_{p=1}^P \Phi(\hat{z}_p) 
%\approx \mathcal{N}(\bs y; \Phi(\hat{\bs z}), \bs Q),
\approx \prod_{p=1}^P \mathcal{N}(y_p; \Phi(z_p), Q_{p,p}) = \mathcal{N}(\bs y; \Phi(\bs z), \bs Q),
...
we could try to estimate the diagonals of $\bs Q$ as
by moment matching with the variance of these Bernoulli distributions
to give $Q_{p,p} = \Phi(z_p)(1 - \Phi(z_p))$.
However, this means that the variance of the approximate likelihood
depends on $\bs z$ and therefore on $p(\bs f)$,
which would make the posterior intractable.

Instead, we estimate the variance of the Bernoulli distribution 
using a local approximation as follows.
First, consider that the conjugate prior for the Bernoulli distribution 
...
The posterior distribution over $\bs f$ given $\bs y$ therefore requires us
to marginalise $\bs f$ itself:





To resolve this, consider that the conjugate prior for the Bernoulli is a beta distribution,



we estimate each variance, $\bs Q_{p,p}$ using a local approximation,
rather than from $p(\bs f)$



\begin{flalign}
p(\bs y) & = \mathbb{E}_{\bs f}[\mathcal{N}(\bs y; \Phi(\bs z), \bs Q)] = \mathcal{N}(\bs y; \Phi(\hat{z}), \tilde{Q} ) & 
\end{flalign}
we moment match the diagonal entries of $\bs Q$ to the expected variance of a bernoulli distribution as follows:
\begin{flalign}
\tilde{Q}_{p,p} & = \mathbb{E}_{\bs f}[\Phi(z_p)(1 - \Phi(z_p))] 
...
We assume a \emph{mean-field} approximation to the posterior over the inducing and training items
...
As we apply mean-field variational inference, 
...
We marginalize $\bs f$ to obtain the factor for $\bs f_m$, taking expectations with respect to $s$:
\begin{flalign}
...
% \label{eq:qw}
% \end{flalign}
\todo{shouldn't we state the whole algorithm as in GPPL?}
