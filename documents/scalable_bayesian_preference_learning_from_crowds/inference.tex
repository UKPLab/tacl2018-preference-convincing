...
+ \bs A_v^T \bs G^T \textrm{diag}(\hat{\bs w}_{c,\bs j}^2 + \bs\Sigma_{c,\bs j,\bs j})\bs Q^{-1} \bs G \bs A_v & \\
\hat{\bs v}_{m,c} = \;\;& \bs S_{v,c} \bs A_v^T \bs G^T \textrm{diag}(\hat{\bs w}_{c,\bs j}) \bs Q^{-1}(\bs y - \Phi\left(\mathbb{E}[\bs z]) + \bs G(\hat{\bs v}_c^T \hat{\bs w}_c)\right), &
\end{flalign}
...
+ \bs A_w^T \left( \bs H^T \textrm{diag}(\hat{\bs v}_{c,\bs a}^2 + \bs S_{c,\bs a, \bs a}) \bs Q^{-1} \bs H^T \right. & \nonumber \\
& \left. + \bs H^T \textrm{diag}(\hat{\bs v}_{c,\bs b}^2 + \bs S_{c,\bs b, \bs b}) 
   \bs Q^{-1} \bs H^T \right) \bs A_w & \\
\hat{\bs w}_{m,c} = \;\;& \bs \Sigma_{c} \bs A_w^T 
\left( \bs H^T\textrm{diag}(\hat{\bs v}_{c,\bs a}) - \bs H^T\textrm{diag}(\hat{\bs v}_{c,\bs b}) \right) \bs Q^{-1} & \nonumber \\
& \left(\bs y - \Phi(\mathbb{E}[\bs z]) + \bs G(\hat{\bs v}_c^T \hat{\bs w}_c)\right), &
\end{flalign}
