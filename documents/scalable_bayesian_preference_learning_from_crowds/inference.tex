...
& \left. + \log p(\bs w_{m,c} | \bs K_{w,mm} )
  - \log q(\bs w_{m,c} ) \big\}
...
\end{flalign}
where $\hat{\bs w}_c$ and $\bs \Sigma_{c}$ also follow the pattern of Equation
Equations \ref{eq:S_stochastic} and \ref{eq:fhat_stochastic}, and their full definitions
are given in Appendix \ref{sec:post_params}, Equations \ref{eq:St} and \ref{eq:hatt}.
...
and the covariance $\bs \Lambda_u^*$ for each users $u$ in the test set:
