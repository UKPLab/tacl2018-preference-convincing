...
\approx \mathcal{N}(\bs y; \Phi(\bs z), \bs Q),
...
$Q_{p,p} = \Phi(z_p)(1 - \Phi(z_p))$ estimated
by moment matching with the variance of the Bernoulli distribution over $y_p$ 
defined by $\Phi(z_p)$.
 
 %why we marganalising f? Do we introduce variational approx here?
Marginalising $\bs f$ gives:
\begin{flalign}
p(\bs y) & = \mathbb{E}_{\bs f}[\mathcal{N}(\bs y; \Phi(\bs z), \bs Q)] = \mathcal{N}(\bs y; \Phi(\hat{z}), \tilde{Q} ) & 
\end{flalign}
The conjugate prior for the Bernoulli is a beta distribution,
